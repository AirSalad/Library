package  
{
	...
	public class EnemyManager extends FlxGroup
	{
		...
		private var enemyWaveOneA:Array = new Array
			...
			{ type: "Wolf", element: "Wind", amount: 1, level: 3, delay: 0 } 
			);
		...
		public function EnemyManager() 
		{
			...
			summoningDelay = new FlxDelay(3000);
			finnishingDelay = new FlxDelay(3000);
		...
		public function bulletExplodeEnemy(bullet:FlxObject, enemy:FlxObject):void
		{
			...
			PlayState.bulletCenter.x = bullet.x;
			PlayState.bulletCenter.y = bullet.y;
			enemy.hurt(bullet.damage);
			PlayState.playerBullets.explosionWeapon.fireAtPosition(enemy.x, enemy.y);
			FlxG.score += bullet.damage;
			bullet.kill();
		}
		...
		public function bulletBombExplodeEnemy(bullet:FlxObject, enemy:FlxObject):void
		{
			...
			PlayState.bulletCenter.y = bullet.y;
			enemy.hurt(bullet.damage);
			PlayState.playerBullets.bombExplosionWeapon.fireAtPosition(enemy.x, enemy.y);
			FlxG.score += bullet.damage;
			bullet.kill();
		}
		
		/*public function reverseEnemy(enemy:FlxObject, wall:FlxObject):void
		{
			trace("Enemy direction switched");
			enemy.maxVelocity;
		}*/
	}
